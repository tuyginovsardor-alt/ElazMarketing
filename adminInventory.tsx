...
(window as any).openProductEditor = async (id?: string) => {
    ...
    if(id) {
        ...
        // Agar yangi qo'shish bo'lsa va oldingi tahrirlangan mahsulot bo'lsa (ID si bor bo'lsa), tozalaymiz
...
(window as any).saveProductFinal = async () => {
    if(!currentEditingProduct.name) return showToast("Nomini kiriting!");
    if(!currentEditingProduct.image_url) return showToast("Rasm yuklang!");
    ...
    if(!res.error) {
        ...
        // Draftni faqat muvaffaqiyatli saqlangach tozalaymiz
        currentEditingProduct = { name: '', price: 0, stock: 0, unit: 'dona', category: 'grocery', description: '', image_url: '' };
        loadAdminProducts();
    } else {
...
(window as any).archiveProduct = async (id: string, currentStatus: boolean) => {
    const msg = currentStatus ? "Mahsulotni faollashtirasizmi?" : "Mahsulotni arxivlaysizmi?";
...
(window as any).deleteProduct = async (id: string) => {
    if(!confirm("Diqqat! Mahsulot bazadan BUTUNLAY O'CHIRILADI. Qaytarib bo'lmaydi! Ishonchingiz komilmi?")) return;
...
async function loadAdminProducts() {
    ...
    if(document.getElementById('st_total')) {
        ...
        const totalVal = prods?.reduce((acc, p) => acc + (p.price * p.stock), 0) || 0;
        document.getElementById('st_val')!.innerText = totalVal.toLocaleString() + " UZS";
    }
                            ...
                            <tr style="border-bottom:1px solid #f8fafc; opacity:${p.is_archived ? '0.6' : '1'}">
                                ...
                                <td style="padding:15px; text-align:right;">
                                    <div style="display:flex; justify-content:flex-end; gap:8px;">
                                        <button title="Tahrirlash" class="btn" style="width:34px; height:34px; border-radius:10px; background:#eff6ff; color:#3b82f6; border:none;" onclick="window.openProductEditor('${p.id}')"><i class="fas fa-pen"></i></button>
                                        <button title="Arxiv/Aktiv" class="btn" style="width:34px; height:34px; border-radius:10px; background:${p.is_archived ? '#f0fdf4' : '#fff7ed'}; color:${p.is_archived ? '#16a34a' : '#ea580c'}; border:none;" onclick="window.archiveProduct('${p.id}', ${p.is_archived})"><i class="fas ${p.is_archived ? 'fa-box-open' : 'fa-box-archive'}"></i></button>
                                        <button title="O'chirish" class="btn" style="width:34px; height:34px; border-radius:10px; background:#fef2f2; color:var(--danger); border:none;" onclick="window.deleteProduct('${p.id}')"><i class="fas fa-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                        `).join('')}
...
function renderEditorUI() {
    ...
    placeholder.innerHTML = `
        <div style="padding-bottom:100px; animation: slideUp 0.3s ease-out;">
            <div style="display:flex; align-items:center; gap:15px; margin-bottom:25px; position:sticky; top:0; background:white; z-index:100; padding:15px 15px; border-bottom:1px solid #f1f5f9;">
            ...
            <div style="padding: 0 15px;">
                <div class="card" style="border-radius:30px; padding:25px; background:white; border:2px solid #f1f5f9;">
                    <div style="text-align:center; margin-bottom:25px;">
                        ...
                        <p style="font-size:0.6rem; color:var(--gray); margin-top:8px; font-weight:700;">MAHSULOT RASMI</p>
                    </div>
