...
export async function renderCourierDashboard(userObj: any, profileObj: any) {
    ...
    try {
        const { data: freshProfile } = await supabase.from('profiles').select('*').eq('id', cUser.id).single();
        if(freshProfile) cProfile = freshProfile;
        ...
        container.innerHTML = `
            <div style="padding-bottom:120px; animation: fadeIn 0.4s ease-out; width:100%;">
                ...
                <div style="display:flex; background:#f1f5f9; padding:6px; border-radius:20px; margin-bottom:25px; gap:5px;">
                    <button onclick="window.switchCourierTab('new')" id="tab_new" style="flex:1.2; height:48px; border-radius:15px; border:none; font-weight:900; font-size:0.65rem; transition:0.3s; position:relative;">YANGI ISH <span id="newBadge" style="display:none; position:absolute; top:8px; right:8px; width:8px; height:8px; border-radius:50%; background:var(--danger);"></span></button>
                    <button onclick="window.switchCourierTab('active')" id="tab_active" style="flex:1; height:48px; border-radius:15px; border:none; font-weight:900; font-size:0.65rem; transition:0.3s;">FAOL</button>
                    <button onclick="window.switchCourierTab('profile')" id="tab_profile" style="flex:1; height:48px; border-radius:15px; border:none; font-weight:900; font-size:0.65rem; transition:0.3s;">SOZLAMALAR</button>
                </div>
...
async function loadTerminalData() {
    ...
    if(!orders?.length) {
        feed.innerHTML = `<div style="text-align:center; padding:5rem 20px; opacity:0.3;"><i class="fas fa-inbox fa-4x"></i><p style="font-weight:900; margin-top:10px;">BUYURTMALAR YO'Q</p></div>`;
        return;
    }
    ...
    feed.innerHTML = orders.map(o => {
        ...
        return `
            <div class="card" style="padding:22px; border-radius:35px; border:2px solid #f1f5f9; background:white; margin-bottom:20px; box-shadow:var(--shadow-sm); animation: slideUp 0.3s ease-out;">
                <div style="display:flex; justify-content:space-between; margin-bottom:15px; border-bottom:1px dashed #f1f5f9; padding-bottom:15px;">
                    <div>
                ...
                <div style="background:#f8fafc; padding:18px; border-radius:24px; margin-bottom:18px;">
                    ...
                    <div style="font-size:0.8rem; font-weight:800; margin-top:10px; color:var(--gray); display:flex; gap:8px;">
                ...
                ${o.payment_method === 'cash' ? `
                    <div style="background:#fef2f2; border:1.5px solid #fee2e2; padding:18px; border-radius:24px; text-align:center; margin-bottom:22px;">
