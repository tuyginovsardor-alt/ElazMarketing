...
export async function renderAdminApplications() {
    ...
    container.innerHTML = `<div style="text-align:center; padding:3rem;"><i class="fas fa-spinner fa-spin fa-2x"></i></div>`;
            ...
            ${apps.map(a => `
                <div class="card" style="padding:22px; border-radius:28px; background:white; position:relative; overflow:hidden;">
                    ...
                    <div style="display:flex; align-items:center; gap:15px; margin-bottom:15px;">
                        ...
                        <div>
                            <div style="font-weight:900; font-size:1rem;">${a.full_name}</div>
                            <div style="font-size:0.75rem; color:var(--gray); font-weight:800;">${a.phone}</div>
                        </div>
                    </div>
                    ...
                    <div style="background:#f8fafc; padding:15px; border-radius:18px; margin-bottom:20px;">
                        <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                            ...
                            <span style="font-size:0.75rem; font-weight:900; color:var(--primary);">${a.transport_type.toUpperCase()}</span>
                        </div>
                        <div style="display:flex; justify-content:space-between;">
                            <span style="font-size:0.65rem; font-weight:800; color:var(--gray);">HUDUDLAR:</span>
                            <span style="font-size:0.7rem; font-weight:900; text-align:right;">${a.work_zones?.join(', ') || 'Markaz'}</span>
                        </div>
                    </div>
