...
export async function renderHomeView() {
    ...
    container.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:20px; padding-bottom:120px; animation: fadeIn 0.4s ease-out;">
            ...
            <!-- MARKETING SLIDER (Siz qo'ygan postlar) -->
            <div id="bannerCarousel" style="position:relative; width:100%; height:190px; overflow:hidden; border-radius:32px; background:#f1f5f9; display:none; box-shadow:var(--shadow-lg);">
                <div id="bannerTrack" style="display:flex; height:100%; transition: transform 0.6s cubic-bezier(0.25, 1, 0.5, 1);"></div>
                <div id="bannerDots" style="position:absolute; bottom:15px; width:100%; display:flex; justify-content:center; gap:10px;"></div>
            </div>
            ...
            <div style="display:flex; overflow-x:auto; gap:12px; padding-bottom:5px; scrollbar-width: none;">
                ${CATEGORIES.map(cat => `
                    <div onclick="window.filterByCategory('${cat.id}')" 
                         style="flex: 0 0 auto; display:flex; flex-direction:column; align-items:center; gap:8px; padding:12px 18px; border-radius:24px; border: 2.5px solid ${activeCategory === cat.id ? 'var(--primary)' : '#f1f5f9'}; background: ${activeCategory === cat.id ? 'var(--primary-light)' : 'white'}; cursor:pointer; min-width:100px; transition:all 0.3s cubic-bezier(0.4, 0, 0.2, 1);">
        ...
        <style>
            .sort-chip { padding: 10px 18px; border-radius: 14px; border: 2px solid #f1f5f9; font-size: 0.75rem; font-weight: 800; color: var(--gray); cursor:pointer; background:white; white-space:nowrap; transition:0.2s; }
            .sort-chip.active { background: var(--primary); color: white; border-color: var(--primary); box-shadow: 0 5px 15px rgba(34,197,94,0.3); }
...
async function loadBanners() {
    ...
    // Banners jadvalidan siz qo'shgan postlarni olish
    ...
    if(data && data.length > 0) {
        ...
        dots.innerHTML = banners.map((_, i) => `
            <div class="dot" style="width:10px; height:6px; border-radius:10px; background:rgba(255,255,255,0.4); transition:all 0.4s;" id="dot-${i}"></div>
        `).join('');
...
async function loadProducts() {
    ...
    if(!prods?.length) {
        grid.innerHTML = `<div style="grid-column: span 2; text-align:center; padding:5rem; color:var(--gray); font-weight:800; opacity:0.5;"><i class="fas fa-search-minus fa-2x mb-3"></i><br>Mahsulot topilmadi</div>`;
        return;
    }

    grid.innerHTML = prods.map(p => `
        <div class="product-card" style="background:white; border-radius:32px; border:1.5px solid #f1f5f9; overflow:hidden; position:relative; cursor:pointer; box-shadow:var(--shadow-sm); transition:all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);" onclick="window.openProductDetailsById(${p.id})">
            <div style="position:relative; width:100%; aspect-ratio:1/1; background:#f8fafc; overflow:hidden;">
                ...
                ${p.discount ? `<div style="position:absolute; top:12px; left:12px; background:var(--danger); color:white; padding:5px 10px; border-radius:12px; font-size:0.65rem; font-weight:900; box-shadow:0 4px 10px rgba(239,68,68,0.3); border:1.5px solid white;">-${p.discount}%</div>` : ''}
                
                <button onclick="event.stopPropagation(); window.handleToggleFav(${p.id}, this)" 
                        style="position:absolute; top:12px; right:12px; width:40px; height:40px; border-radius:14px; background:rgba(255,255,255,0.9); border:none; color:${favIds.includes(p.id) ? 'var(--danger)' : '#cbd5e1'}; font-size:1.1rem; cursor:pointer; backdrop-filter:blur(10px); display:flex; align-items:center; justify-content:center; box-shadow:var(--shadow-sm);">
                    <i class="${favIds.includes(p.id) ? 'fas' : 'far'} fa-heart"></i>
                </button>

                <!-- QUICK ADD BUTTON -->
                <button onclick="event.stopPropagation(); window.quickAddToCart(${p.id})" 
                        style="position:absolute; bottom:12px; right:12px; width:48px; height:48px; border-radius:16px; background:var(--gradient); border:2px solid white; color:white; font-size:1.4rem; cursor:pointer; box-shadow: 0 8px 20px rgba(34,197,94,0.3); display:flex; align-items:center; justify-content:center; transform:translateY(0); transition:0.2s;">
            ...
            <div style="padding:18px;">
                ...
                <div style="font-size:0.65rem; color:var(--gray); font-weight:800; margin-top:4px; text-transform:uppercase; letter-spacing:0.5px;">${p.unit}</div>
                <div style="display:flex; align-items:center; gap:8px; margin-top:10px;">
                    <span style="font-weight:900; color:var(--primary); font-size:1.15rem;">${p.price.toLocaleString()}</span>
                    ${p.discount ? `<span style="font-size:0.75rem; color:var(--gray); text-decoration:line-through; opacity:0.6;">${Math.round(p.price * 1.2).toLocaleString()}</span>` : ''}
                </div>
            </div>
        </div>
    `).join('');
