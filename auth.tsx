...
export function renderAuthView(mode: AuthMode = 'login', extraData?: any) {
    ...
    container.innerHTML = `
        <div style="padding: 20px; display: flex; flex-direction: column; min-height: 100%; max-width: 420px; margin: 0 auto; animation: fadeIn 0.5s ease-out;">
            ...
            <!-- Main Form Card -->
            <div style="background: white; border-radius: 35px; padding: 5px; box-shadow: 0 20px 40px rgba(0,0,0,0.05);">
                
                <!-- Internal Tabs -->
                ${(mode === 'login' || mode === 'email_auth') ? `
                    <div style="display: flex; background: #f1f5f9; padding: 5px; border-radius: 20px; margin: 10px;">
                        <button onclick="renderAuthView('login')" style="flex: 1; height: 45px; border: none; border-radius: 16px; font-weight: 800; font-size: 0.7rem; cursor: pointer; transition: 0.3s; background: ${mode === 'login' ? 'white' : 'transparent'}; color: ${mode === 'login' ? 'var(--primary)' : 'var(--gray)'}; box-shadow: ${mode === 'login' ? '0 4px 10px rgba(0,0,0,0.05)' : 'none'};">TELEFON</button>
                        <button onclick="renderAuthView('email_auth')" style="flex: 1; height: 45px; border: none; border-radius: 16px; font-weight: 800; font-size: 0.7rem; cursor: pointer; transition: 0.3s; background: ${mode === 'email_auth' ? 'white' : 'transparent'}; color: ${mode === 'email_auth' ? 'var(--primary)' : 'var(--gray)'}; box-shadow: ${mode === 'email_auth' ? '0 4px 10px rgba(0,0,0,0.05)' : 'none'};">EMAIL</button>
                    </div>
                ` : ''}

                <div style="padding: 20px;">
                    <div style="margin-bottom: 25px; text-align: center;">
                        <h2 style="font-size: 1.4rem; font-weight: 900; color: var(--text);">
                            ${mode === 'login' ? 'Xush kelibsiz! 👋' : 
                              mode === 'register_phone' ? 'Ro\'yxatdan o\'tish 📝' :
                              mode === 'verify_otp' ? 'Telegram kodi 📩' :
                    ...
                    <div style="display: flex; flex-direction: column; gap: 15px;">
                        ...
                        ${mode === 'email_auth' ? `
                            <div class="input-group">
                                <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--gray); margin-bottom: 6px;">GMAIL MANZIL</label>
                                <input type="email" id="authEmail" placeholder="example@gmail.com" style="width: 100%; height: 60px;">
                            </div>
                            <div class="input-group">
                                <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--gray); margin-bottom: 6px;">PAROL</label>
                                <input type="password" id="authEmailPass" placeholder="Kamida 6 belgi" style="width: 100%; height: 60px;">
                            </div>
                        ` : ''}

                        ${mode === 'register_phone' ? `
                            <div class="input-group">
                                <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--gray); margin-bottom: 6px;">YANGI TELEFON</label>
                                <div style="position: relative;">
                                    <span style="position: absolute; left: 18px; top: 50%; transform: translateY(-50%); font-weight: 800; color: var(--text);">+998</span>
                        ...
                        ${mode === 'verify_otp' ? `
                            <div class="input-group" style="text-align: center;">
                                <input type="number" id="authOtp" placeholder="000000" maxlength="6" style="width: 100%; height: 75px; border-radius: 20px; border: 3px solid var(--primary); font-size: 2rem; font-weight: 900; text-align: center; letter-spacing: 8px;">
                                <div id="otpStatusDisplay" style="margin-top:10px; font-size:0.7rem; font-weight:800; color:var(--gray);">Bot orqali kod yuborilmoqda...</div>
                            </div>
                        ` : ''}
                        ...
                        ${mode === 'complete_profile' ? `
                            ...
                            <div class="input-group">
                                <label style="display: block; font-size: 0.65rem; font-weight: 800; color: var(--gray); margin-bottom: 6px;">PAROL YARATING</label>
                                <input type="password" id="regPass" placeholder="Kamida 6 ta belgi" style="width: 100%; height: 60px;">
                            </div>
                        ` : ''}

                        <button class="btn btn-primary" id="authSubmitBtn" style="width: 100%; height: 60px; border-radius: 20px; margin-top: 5px;" onclick="executeAuthAction('${mode}', ${JSON.stringify(extraData)})">
                            ${mode === 'login' || mode === 'email_auth' ? 'KIRISH' : 
                              mode === 'register_phone' ? 'KODNI OLISH' : 
                              mode === 'verify_otp' ? 'TASDIQLASH' : 'SAQLASH VA KIRISH'}
                        </button>

                        <!-- GOOGLE VA O'TISH TUGMALARI -->
            ...
            <p style="text-align: center; font-size: 0.7rem; color: #cbd5e1; margin-top: 30px; font-weight: 800; letter-spacing: 1px;">ELAZ MARKET SECURE AUTH</p>
        </div>
    `;
...
(window as any).signInWithGoogle = async () => {
    try {
        ...
        showToast("Google xatosi: " + err.message);
    }
};
...
(window as any).executeAuthAction = async (mode: AuthMode, extraData?: any) => {
    ...
    try {
        if (mode === 'login') {
            ...
            if (!email.includes('@') || pass.length < 6) throw new Error("Email yoki parol noto'g'ri!");
            ...
            if (signInError) {
                ...
                showToast("Ro'yxatdan o'tdingiz! Gmailingizni tasdiqlang.");
            } else {
                showToast("Xush kelibsiz! 🎉");
                await checkAuth();
            }
            ...
            if (name.length < 2 || pass.length < 6) throw new Error("Ism (min 2) yoki parol (min 6) kamlik qiladi!");

            const phone = extraData.phone;
            const { data: auth, error: signUpErr } = await supabase.auth.signUp({ email: phone + '@elaz.uz', password: pass });
            if (signUpErr) throw signUpErr;

            if (auth.user) {
                await supabase.from('profiles').update({ id: auth.user.id, first_name: name, otp_status: 'verified' }).eq('phone', phone);
            }
