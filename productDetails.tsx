...
export function renderProductDetails(p: any) {
    ...
    currentQty = 1;
    const mainImg = p.images?.[0] || p.image_url || "https://via.placeholder.com/600";
    const allImgs = Array.isArray(p.images) ? p.images : [mainImg];

    placeholder.innerHTML = `
        <div style="padding-bottom:120px;">
            <div style="position:relative; width:100%; aspect-ratio:1/1; border-radius:30px; overflow:hidden; background:#f8fafc; margin-bottom:25px;">
            ...
            <div style="padding:0 5px;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:15px;">
                    <div>
                        <span style="background:var(--primary-light); color:var(--primary); font-size:0.7rem; font-weight:900; padding:4px 12px; border-radius:8px; text-transform:uppercase; border:1px solid rgba(34,197,94,0.1);">${p.category}</span>
                        <h2 style="font-weight:900; font-size:1.6rem; margin-top:8px; color:var(--text); letter-spacing:-0.5px;">${p.name}</h2>
                        <p style="color:var(--gray); font-weight:700; font-size:0.9rem;">${p.brand || 'Mahsulot'}</p>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-weight:900; font-size:1.6rem; color:var(--primary);">${p.price.toLocaleString()} UZS</div>
                        <div style="font-size:0.75rem; color:var(--gray); font-weight:700;">1 ${p.unit} uchun</div>
                    </div>
                </div>
                ...
                <div style="display:flex; align-items:center; gap:20px; margin-bottom:25px; background:#f8fafc; padding:12px 20px; border-radius:18px; width:fit-content;">
                    <span style="font-weight:800; color:var(--gray); font-size:0.9rem;">Miqdori:</span>
                ...
                <div class="card" style="border-radius:24px; padding:20px; border:1px solid #f1f5f9; background:#f8fafc; margin-bottom:25px;">
                    <h4 style="font-weight:900; margin-bottom:10px; font-size:1rem;">Mahsulot tavsifi</h4>
                    <p style="font-size:0.9rem; color:var(--gray); line-height:1.6; font-weight:600;">${p.description || "Ushbu mahsulot haqida qo'shimcha ma'lumot berilmagan."}</p>
                </div>
